        ...
        $(window).load(function(){
          ...
          $.getJSON("allPatients.php", function( data ) {
           ...
           $.getJSON("allseverite.php", function( data ) {
            var cont = "";
            $.each( data, function( key, val ) {
              cont += '<span color="red" class="label label-primary"><span class="glyphicon glyphicon-user" aria-hidden="true"> ' + val['NbPatient'] + '</span></span>';
            });
      ...
      function showAttente(hopital, departement) {
        $.getJSON( "attente.php?ID=" + departement, function( data ) {
          ...
          $.each( data, function( key, val ) {
            ...
            content += '<div class="panel-body">Patients en attente: ' + val['Attente'] + '<br>Temps d\'attente prevue: ' + secondsTimeSpanToHM(Math.abs(val['MOYENNE'] * val['Attente'])) +  '<br>Moyenne d\'attente: ' + secondsTimeSpanToHM(Math.abs(val['MOYENNE'])) + ' heures</div></div>';
